...
The TOON Protocol **store** — NIP-90 **kind:5094** Arweave blob storage, plus one optional job kind: **kind:5095** ArNS, gated by `ARNS_DVM_SOLANA_SECRET_KEY`, which carries two ops — `op=buy`, the brokered name purchase (`src/arns-buy-handler.ts`), and `op=prepare`, which composes an unsigned ANT-spawn transaction the client signs and the gas station pays to broadcast (`src/arns-ant-prepare.ts`). `op` defaults to `buy`. The gas-station kinds (5096 Solana, 5098 EVM) moved to **[toon-protocol/gas-station](https://github.com/toon-protocol/gas-station)**. Built from `Dockerfile.store` over `src/entrypoint-store.ts`, which wraps `@toon-protocol/sdk`'s `createArweaveDvmHandler`: upload the blob to Arweave via Turbo, return the tx id. This is a **container, not an npm package** (`@toon-protocol/store`, kept private). It runs as a payment-oblivious `POST /store` backend (`src/store-backend.ts`) behind the connector, which is the front-of-app payment proxy and reverse-proxies to it (RouteTermination).
