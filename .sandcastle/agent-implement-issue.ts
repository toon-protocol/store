...
//   APP_ID, APP_PRIVATE_KEY   the same GitHub App GH_TOKEN is minted from.
//                             Used to mint a FRESH token immediately before
//                             each push, because installation tokens expire
//                             after one hour and long runs pushed with a dead
//                             credential — see toon-meta#248 and
...
// store is a single-package pnpm repo: install with the committed lockfile so
// the sandbox resolves the exact dependency tree. Mirrors main.ts (we
...
// ---------------------------------------------------------------------------
// Pushing with a credential that is fresh AT THE MOMENT OF THE PUSH (toon-meta#248)
// ---------------------------------------------------------------------------
 ...
 * calls that follow (`issue view`, `pr list`, `pr create`, `repo view`, `api`)
 * authenticate from `process.env` and expire on exactly the same one-hour clock.
 *
...
async function pushBranch(
  ...
  // ONE place decides what a failure means, so every failure mode below reads
  // as a single line: warn-and-carry-on for the best-effort early publish,
  // throw for the final push.
  ...
  try {
    ...
    return fail(`could not obtain a push credential: ${detail}`);
  }
  ...
  try {
    ...
    // phases that follow. The `try` opens BEFORE the staging write so a partial
    // or failed stage is swept too, not just a completed push.
...
try {
  ...
  // PUBLISH EARLY (toon-meta#248). The implementer has committed; get those
  // commits onto origin NOW rather than after the reviewer. Two reasons:
  //   1. Recoverability — a run that dies during review (timeout,
  //      cancellation, runner death) leaves the completed implementation on a
  //      remote branch instead of losing it with the container.
