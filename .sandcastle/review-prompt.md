...
2. Run store's gate to ensure nothing is broken — `pnpm run typecheck`, `vitest run`, and `pnpm run build`. store has NO lint step (no eslint config); do not run eslint.
