 ...
 *
 * Bundles entrypoint-store.ts into a self-contained ESM file. Dynamically-
 * required packages (ethers, express) are marked external since they use
 * variable `require()` calls in the connector's `requireOptional()` that
 * esbuild can't resolve.
 *
...
const result = await esbuild.build({
  ...
  external: ['ethers', 'express', '@ardrive/turbo-sdk', 'arweave', 'o1js', '@solana/kit', '@solana-program/token', '@toon-protocol/mina-zkapp', 'mina-signer', 'mina-fungible-token', 'socks-proxy-agent', 'fastify', '@fastify/cors', '@noble/curves'],
